...
for ((i=0; i<num_cases; i++)); do
    ...
    if script_output=$(./run.sh "$trip_duration" "$miles_traveled" "$receipts_amount" 2>/dev/null); then
        ...
        if [[ $output =~ ^-?[0-9]+\.?[0-9]*$ ]]; then
            ...
            # Calculate absolute error using Python
            error=$(python -c "print(abs($actual - $expected))")
            ...
            if python -c "exit(0 if $error < 0.01 else 1)"; then
                exact_matches=$((exact_matches + 1))
            fi
            
            # Check for close match (within $1.00)
            if python -c "exit(0 if $error < 1.0 else 1)"; then
                close_matches=$((close_matches + 1))
            fi
            
            # Update total error
            total_error=$(python -c "print($total_error + $error)")
            
            # Track maximum error
            if python -c "exit(0 if $error > $max_error else 1)"; then
                max_error="$error"
...
if [ $successful_runs -eq 0 ]; then
    ...
    avg_error=$(python -c "print(f'{$total_error / $successful_runs:.2f}')")
    
    # Calculate percentages
    exact_pct=$(python -c "print(f'{$exact_matches * 100 / $successful_runs:.1f}')")
    close_pct=$(python -c "print(f'{$close_matches * 100 / $successful_runs:.1f}')")
    ...
    score=$(python -c "print(f'{$avg_error * 100 + ($num_cases - $exact_matches) * 0.1:.2f}')")
